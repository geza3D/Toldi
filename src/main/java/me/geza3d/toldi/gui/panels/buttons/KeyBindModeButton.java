...
public class KeyBindModeButton extends ButtonPanel {
	...
	protected void onRender(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		fill(matrices, x + width/2 - 5 - Toldi.TEXTRENDERER.getWidth(text)/2 - 3, y + height / 2 - 5, x + width/2 - 5 + Toldi.TEXTRENDERER.getWidth(text) / 2 + 3, y + height / 2 + 6, GuiValues.c6);
		drawCenteredText(matrices, Toldi.TEXTRENDERER, text, x + width / 2 - 5, y + height / 2 - 4, GuiValues.c7);
	}
